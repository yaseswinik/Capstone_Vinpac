# -*- coding: utf-8 -*-
"""
Created on Fri May  7 10:42:46 2021
...
columnsindf = list(Rules_mapping_20.columns)

for column in columnsindf:
    if "_0" in column:
        Rules_mapping_20.drop(column, axis=1, inplace=True)
    elif "_7" in column:
        Rules_mapping_20.drop(column, axis=1, inplace=True)
    elif "_8" in column:
        Rules_mapping_20.drop(column, axis=1, inplace=True)
...
#s1_20 = Rules_mapping_dummies_20    

ix_20 =  Rules_mapping_20[(Rules_mapping_20.T!=0).any()]
 
ix_20.drop('Item_No', inplace=True, axis=1) 
 
############### choose max_len either 2 or 3--- decide from lit review
       
frequent_itemsets_20 = apriori(ix_20, min_support= 0.002, max_len = 3, use_colnames=True)
