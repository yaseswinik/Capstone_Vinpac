# -*- coding: utf-8 -*-
"""
Created on Fri May  7 19:16:08 2021
...
columnsindf = list(Rules_mapping_19.columns)

for column in columnsindf:
    if "_0" in column:
        Rules_mapping_19.drop(column, axis=1, inplace=True)
    elif "_7" in column:
        Rules_mapping_19.drop(column, axis=1, inplace=True)
    elif "_8" in column:
        Rules_mapping_19.drop(column, axis=1, inplace=True)
...
ix_19 =  Rules_mapping_19[(Rules_mapping_19.T!=0).any()]
 
ix_19.drop('Item_No', inplace=True, axis=1) 
 
############### choose max_len either 2 or 3--- decide from lit review
       
frequent_itemsets_19 = apriori(ix_19, min_support= 0.002, max_len = 3, use_colnames=True)
...
#rules_sorted_1.to_csv("rules_sorted_1_1920.csv")
